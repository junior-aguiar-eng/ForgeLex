...
export const persistenceMigrations: readonly SqlMigration[] = [
  ...
  {
    id: 'persistence-0023-account-closure',
    statements: [
      `CREATE TABLE IF NOT EXISTS account_closures (
        ...
        identity_removed_at TEXT,
        completed_at TEXT,
        next_attempt_at TEXT,
        attempt_count INTEGER NOT NULL DEFAULT 0,
        last_error_code TEXT
      );`,
      `CREATE UNIQUE INDEX IF NOT EXISTS account_closures_subject_idempotency_idx ON account_closures(subject_hash, idempotency_key_hash);`,
      `CREATE INDEX IF NOT EXISTS account_closures_status_due_idx ON account_closures(status, next_attempt_at);`,
      `CREATE TABLE IF NOT EXISTS account_closure_steps (
        id TEXT PRIMARY KEY,
        closure_id TEXT NOT NULL REFERENCES account_closures(id),
      ...
      `CREATE UNIQUE INDEX IF NOT EXISTS account_closure_steps_closure_type_idx ON account_closure_steps(closure_id, step_type);`,
      `CREATE INDEX IF NOT EXISTS account_closure_steps_due_idx ON account_closure_steps(status, next_attempt_at);`,
