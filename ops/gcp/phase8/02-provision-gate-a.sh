...
gcloud run deploy "$SERVICE" --image="$IMAGE_URI@$DIGEST" --region="$REGION" --platform=managed --service-account="$RUNTIME_EMAIL" --add-cloudsql-instances="$CONNECTION_NAME" --set-secrets="DATABASE_URL=forgelex-hml-database-url:$DATABASE_SECRET_VERSION,FORGELEX_WEBHOOK_MASTER_KEY=forgelex-hml-webhook-master-key:$WEBHOOK_SECRET_VERSION,FORGELEX_METRICS_TOKEN=forgelex-hml-metrics-token:$METRICS_SECRET_VERSION" --set-env-vars="NODE_ENV=production" --port=8080 --cpu=1 --memory=512Mi --concurrency=20 --min-instances=0 --max-instances=2 --timeout=60s --allow-unauthenticated --labels="$LABELS" --project="$PROJECT_ID"
