...
import { ApiRequestError, getBillingAccount, getBillingTransactions, requestApi, type BillingAccount, type BillingInvoice, type BillingPurchase, type BillingTransaction, type PaymentMethod } from '../api-client';
...
export const CreditsScreen: React.FC = () => {
  ...
  if (!data) return <div className="py-8 md:py-12"><div className="page-container"><section className="surface max-w-2xl space-y-4 p-6 sm:p-8"><p className="eyebrow">Conta</p><h1 className="font-editorial text-3xl font-bold text-stone-900">{billingState === 'unavailable' ? 'Créditos indisponíveis neste ambiente' : 'Não foi possível carregar os créditos'}</h1><p className="text-sm leading-relaxed text-stone-600">{billingState === 'unavailable' ? 'O faturamento não está habilitado neste ambiente. Saldos, pagamentos e recibos não podem ser consultados aqui.' : 'Não foi possível consultar os dados de faturamento. Tente novamente em alguns instantes.'}</p>{billingState === 'error' && message && <p role="alert" className="rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-800">{message.text}</p>}<button type="button" onClick={() => void load()} className="btn-secondary"><RefreshCw className="h-4 w-4" aria-hidden="true" />Tentar novamente</button></section></div></div>;
